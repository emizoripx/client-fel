...
class UpdateProductLinesCodigoProducto
{
    /**
     * Run the patch.
     *
    ...
    public function run()
    {
        ...
        \App\Models\Company::select('id')->cursor()->each(function ($company) use ($hashid) {
            ...
            $init = microtime(true);
            \App\Models\Invoice::whereCompanyId($company->id)->cursor()->each(function ($invoice) use ($products_encoded, &$counterInvoicesOk, &$counterInvoicesNotOk, &$invoiceWithProblems) {
