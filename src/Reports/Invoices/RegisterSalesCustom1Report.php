...
class RegisterSalesCustom1Report extends BaseReport implements ReportInterface
{
    ...
    public function addSelectColumns($query)
    {
        ...
        if ($this->revocated_zero) {
            $query->selectRaw('(@counter := @counter +1) as num,"2" as especificaciones, fechaEmision,numeroFactura,cuf,numeroDocumento,complemento,nombreRazonSocial, if(codigoEstado=690 ||  codigoEstado=908,round(montoTotal+descuentoAdicional+montoGiftCard,2),"0.00") as importeTotal, "0.00" as importeIce,"0.00" as importeIEHD, "0.00" as importeIPJ,"0.00" as tasas,"0.00" as otros,"0.00" as exportaciones,"0.00" as tasaCero, if(codigoEstado=690 ||  codigoEstado=908,round(montoTotal,2),"0.00") as subTotal, if(codigoEstado=690 ||  codigoEstado=908,descuentoAdicional,"0.00") as descuentoAdicional, if(codigoEstado=690 ||  codigoEstado=908,  round(montoGiftCard,2),"0.00") as montoGiftCard,"0.00" as bonificaciones, if(codigoEstado=690 ||  codigoEstado=908,round(montoTotalSujetoIva,2),"0.00") as baseCreditoFiscal, if(codigoEstado=690 ||  codigoEstado=908,round(montoTotalSujetoIva*0.13,2),"0.00") as debitoFiscal, if(codigoEstado=690 ||  codigoEstado=908,"V","A") as estado, "" as codigo_control, "0" as tipoVenta, codigoSucursal');
        } else {
            $query->selectRaw('(@counter := @counter +1) as num,"2" as especificaciones, fechaEmision,numeroFactura,cuf,numeroDocumento,complemento,nombreRazonSocial,round(montoTotal+descuentoAdicional+montoGiftCard,2) as importeTotal,"0.00" as importeIce,"0.00" as importeIEHD, "0.00" as importeIPJ,"0.00" as tasas,"0.00" as otros,"0.00" as exportaciones,"0.00" as tasaCero,round(montoTotal,2) as subTotal, descuentoAdicional,round(montoGiftCard,2) as montoGiftCard, round(montoTotalSujetoIva,2) as baseCreditoFiscal,round(montoTotalSujetoIva*0.13,2) as debitoFiscal, if(codigoEstado=690 ||  codigoEstado=908,"V","A") as estado, "" as codigo_control, "0" as tipoVenta, codigoSucursal,"0.00" as bonificaciones');
        }
